...
parser.add_argument('--model_name', type=str, default='cnn.pt')
...
def eval_model(model, eval_loader, epoch=None, name='VALIDATION'):
		...
		with torch.set_grad_enabled(False):
			for eval_feats, eval_labels, eval_lengths in eval_loader:
				...
				eval_predictions = model(eval_feats.float()) # the output for every batch's input containing the softmax probabilities
...
def train_model(train_loader, val_loader, max_epochs, input_dim, output_dim, dropout=0.0):
	# Load Model
	...
	# Load Model
	model = ConvNet(input_dim=input_dim, output_dim=output_dim, num_classes=10, dropout=0.0, device=args.cuda) 
	...
	for epoch in range(max_epochs):
		...
		for batch_feats, batch_labels, batch_lengths in train_loader:
			...
			# Model's output
			predictions = model(batch_feats.float()) # float() is used to dtype bug
...
if __name__ == "__main__":
	...
	if args.train:
		...
		TrainLoss, ValLoss, TrainAcc, ValAcc = train_model(train_loader, val_loader, max_epochs=args.epochs, input_dim=28, output_dim=10, dropout=0.7)
		# Learning Curve
